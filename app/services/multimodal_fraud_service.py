...
class MultimodalFraudService:
    ...
    def score(self, claim: ClaimInput) -> ScoringResponse:
        ...
        image_score, image_findings = self._image_score(analyzed_files, claim)
        ...
        return ScoringResponse(
            ...
            confidence_score=self._confidence(analyzed_files, claim, summary),
    ...
    def _image_score(
        ...
        return MultimodalFraudService._blend_encoder_score(
            min(100.0, score),
            image_files,
        ), findings
    ...
    def _confidence(
        ...
        available = {file.document_type for file in files}
        score = 35.0
        score += 10 if DocumentType.CLAIM_FORM in available else 0
        score += 12 if DocumentType.DAMAGE_PHOTO in available else 0
        score += 10 if DocumentType.REPAIR_INVOICE in available else 0
        score += 8 if DocumentType.ACCIDENT_REPORT in available else 0
        score += 8 if claim.claim_amount < 5000 or DocumentType.POLICE_REPORT in available else 0
        score += min(9, summary.text_extraction_successful * 3)
        score += min(8, summary.image_metadata_extracted * 4)
        score += min(10, summary.llm_files_encoded * 3)
